...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      // Timestamp of the new block's creation
      ...
      if (height >= 0) {
        ...
        // assigning previous block's hash
        block.previousBlockHash = self.chain[self.height].hash;
        // Assign new hash to the new block
    ...
    return new Promise(async (resolve, reject) => {
        if (!self.chain) {
          ...
          for (let i = 1; i < self.chain.length; i++) {
            ...
            if (prevBlock.hash !== currBlock.previousBlock) {
              errorLog.push({
                index: i,
                error:
                  "Hash of Previous Block does not match the current block's hash",
              });
            }
          }
